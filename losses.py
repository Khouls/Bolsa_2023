...
def hbb_iou(x1, y1, w1, h1, x2, y2, w2, h2):
    '''
...
def hbb_yolov2_loss(detector_mask, matching_true_boxes, class_one_hot, true_boxes_grid, y_pred, info=False):
    '''
    ...
    wh_loss = LAMBDA_COORD * K.sum(detector_mask * K.square(K.sqrt(matching_true_boxes[...,2:4]) - K.sqrt(pred_wh))) / (nb_detector_mask + 1e-6)
    ...
    w2 = pred_wh[...,0]
    h2 = pred_wh[...,1]
    ious = hbb_iou(x1, y1, w1, h1, x2, y2, w2, h2)
...
# test loss
def test_loss(generator, model, loss_function):
    # get batch
    img, detector_mask, matching_true_boxes, class_one_hot, true_boxe_grid = next(generator)
    ...
    # loss info
    loss, sub_loss = loss_function(detector_mask, matching_true_boxes, class_one_hot, true_boxe_grid, y_pred, info = True)

    plt.savefig(f'losses_test/losses.png')